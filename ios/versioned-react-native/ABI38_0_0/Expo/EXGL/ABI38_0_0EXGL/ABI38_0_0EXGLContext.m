...
#import <ABI38_0_0EXGL/ABI38_0_0EXGLObjectManager.h>
...
@interface ABI38_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI38_0_0UMModuleRegistry *moduleRegistry;
@property (nonatomic, weak) ABI38_0_0EXGLObjectManager *objectManager;

@end

@implementation ABI38_0_0EXGLContext

- (instancetype)initWithDelegate:(id<ABI38_0_0EXGLContextDelegate>)delegate andModuleRegistry:(nonnull ABI38_0_0UMModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    self.delegate = delegate;

    _moduleRegistry = moduleRegistry;
    _objectManager = (ABI38_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI38_0_0EXGLObjectManager class]];
...
- (void)initialize:(void(^)(BOOL))callback
{
  ...
  if (jsContextRef) {
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI38_0_0UMLogWarn(@"ABI38_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ABI38_0_0React-native/docs/debugging.html)? ABI38_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI38_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI38_0_0UMErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI38_0_0UMErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI38_0_0UMUtilities screenScale];
